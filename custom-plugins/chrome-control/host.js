return {
  ...
  apply(ctx) {
    ...
    const HELPER_REL = '.dsh-chrome/chrome-helper.mjs'
    ...
    async function helperPath() {
      return (await baseDir()) + '/' + HELPER_REL
    }

    async function ensureHelperFile() {
      const path = await helperPath()
      const target = await fsSvc.resolve(path)
      try {
        await fsSvc.readText(target)
      } catch {
        throw new Error('chrome helper missing at ' + path + '; recreate it from the session workspace before using chrome_* tools')
      }
      return path
    }
    ...
    async function spawnHelper() {
      ...
      const helperPathResolved = await ensureHelperFile()
      const node = await subprocess.resolveExecutable('node')
      const handle = subprocess.spawn({
        argv: [node, helperPathResolved],
        cwd: await baseDir(),
    ...
    for (const tool of TOOLS) {
      disposers.push(harness.registerTool(ctx, contentTool(tool.name, tool.description, tool.parameters, serial(tool.exec))))
    }
    ctx.effect(() => () => {
