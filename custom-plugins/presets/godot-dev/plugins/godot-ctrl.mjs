// godot-ctrl 静态版 — 持久化到 Godot 预设的插件行（name: ./plugins/godot-ctrl.mjs）。
// 与动态版（custom-plugins/godot-ctrl/host.js）同一套逻辑，仅注册方式不同：
//   - 动态版：harness.defineTool/harness.registerTool（沙箱 marker 校验）
//   - 静态版：直接构造 ToolDefinition 经 ctx.tools.register（预设插件常规路径，无 marker 守卫）
// 纯 ESM，无任何 import（避免从用户主目录解析 node_modules 的问题）。
// 依赖 Host 能力：subprocess（curl → MCP streamable-HTTP）、fs（截图落盘）、tools（注册）。

export const name = 'godot-ctrl'
...
export function apply(ctx) {
  ...
  // 静态预设插件：直接构造 ToolDefinition（raw），经 ctx.tools.register 注册。
  ...
  tools.push(makeTool(
    ...
    '节点树操作（删除/复制/重命名/排序/重挂/分组）。op 必填，params 传该 op 的参数。常用：delete({path})、duplicate({path,name})、rename({path,new_name})、move({path,index})、reparent({path,new_parent})、add_to_group({path,group})、remove_from_group({path,group})、get_children({path})、get_groups({path})。不能删除/复制场景根；scene_file 可选作为场景守卫。',
    {
      ...
      params: { type: 'object', additionalProperties: true, description: 'op 的参数，如 {"path":"/Main/Enemy","new_name":"Boss"}' },
    },
    ['op'],
    async function (args, exec) {
  ...
  tools.push(makeTool(
    ...
    '运行时游戏检查与输入模拟（需先 godot_project_run 启动，并用 godot_status 确认 live）。op 必填。常用：get_scene_tree({depth,root_path})、get_node_info({path,include_properties})、get_ui_elements({root_path,include_hidden,include_disabled,max_depth})（UI 测试）、input_key({key,pressed,echo})、input_mouse({event:"motion"|"button",position:{x,y},button,pressed})、input_gamepad({device,control:"button"|"axis",index,pressed,value})、input_action({action,pressed,strength})、input_sequence({steps:[{at_frame,action,pressed,strength}],settle_frames})（帧级时序输入，推荐）、input_state({actions})。',
    {
      ...
      params: { type: 'object', additionalProperties: true, description: 'op 的参数' },
    },
    ['op'],
    async function (args, exec) {
  ...
  tools.push(makeTool(
    ...
    {
      commands: { type: 'array', items: { type: 'object', additionalProperties: true }, description: '[{"command":"create_node","params":{...}}, ...]' },
  ...
  console.log('[godot-ctrl] 已注册 ' + registered + ' 个 Godot 控制工具（目标 ' + state.baseUrl + '）')
}
